...
- 支持 OpenAI、Claude 和 Gemini 协议链路，以及流式和非流式请求。

  Supports OpenAI, Claude, and Gemini protocol paths for streaming and non-streaming requests.

- 流式 tool call 在结束帧到达前中断时，可从未完成流状态恢复已接收的推理内容。

  Recovers received reasoning from unfinished stream state when a streaming tool call is interrupted before its terminal frame.
...
在 CPA 配置文件的 `plugins.configs` 下添加以下配置：

Add the following plugin configuration under `plugins.configs` in CPA:
...
| 配置项 / Field | 默认值 / Default | 说明 / Description |
|---|---|---|
